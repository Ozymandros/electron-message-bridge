...
describe('AdapterMissingError', () => {
  ...
  it('exposes .adapterName field', () => {
    const err = new AdapterMissingError('@electron-ipc-helper/adapter-assemblyscript');
    expect(err.adapterName).toBe('@electron-ipc-helper/adapter-assemblyscript');
  });

  it('message includes the adapter name and an install hint', () => {
    const err = new AdapterMissingError('@electron-ipc-helper/adapter-assemblyscript');
    expect(err.message).toContain('@electron-ipc-helper/adapter-assemblyscript');
